firstrun(){
  ...
  if [ ! -f $datadir/profiles.json ]; then
    ...
    jq 'to_entries | map(if .value.host == null then .value = {"host":""} + .value else . end) | from_entries' profiles.json > $DATADIR/INPUT.tmp && mv $DATADIR/INPUT.tmp $DATADIR/profiles.json; chmod  600 $DATADIR/profiles.json
  fi
